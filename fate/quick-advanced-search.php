...
$searchName = filter_var($_GET['search-name'], FILTER_SANITIZE_SPECIAL_CHARS);
$searchAka = filter_var($_GET['search-aka'], FILTER_SANITIZE_SPECIAL_CHARS);
$searchClass = filter_var($_GET['search-class'], FILTER_SANITIZE_SPECIAL_CHARS);
$searchIllustrator = filter_var($_GET['search-illustrator'], FILTER_SANITIZE_SPECIAL_CHARS);

if (!empty($searchName) || !empty($searchAka) || !empty($searchClass) || !empty($searchIllustrator)) {
    $results = ServantDAO::advancedSearch($searchName, $searchAka, $searchClass, $searchIllustrator);
}
require 'header.php';
...
<div class="bg-dark container px-5 my-5" style="--bs-bg-opacity: .7">
    <h1 class="text-white px-5 pt-3">Advanced Filter:</h1>
